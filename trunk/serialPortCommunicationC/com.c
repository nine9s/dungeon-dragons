...
//			sprintf(message,"%x",In1);
                        fputs(message,output);
			strcat(tempChar,message);
			//fputs(message,tempChar);
			fflush(output);
		...
		printf("%s",tempChar);
		sprintf(tempChar1,"(wget -q \"http://localhost/intelDesignExpo/updatePosition.php?latitude=34.5&longitude=-118.345&id=%s\")",tempChar);
      printf("%s",tempChar1);
	system(tempChar1);                  
            }  //end if res>0
//            buf[res]=0;
